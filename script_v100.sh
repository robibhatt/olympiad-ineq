#!/bin/bash
#SBATCH --job-name=generate_ineq_v100
...
###############################################################################
# Validate OUTPUT_DIR is set (should be passed from generate_v100.sh)
###############################################################################
if [ -z "${OUTPUT_DIR:-}" ]; then
    echo "ERROR: OUTPUT_DIR not set. Run via generate_v100.sh"
    exit 1
fi
...
###############################################################################
# --- Run data generation with V100 config ---
###############################################################################
python generate.py --config-name=config_v100_test "hydra.run.dir=${OUTPUT_DIR}"

echo "V100 data generation complete."
